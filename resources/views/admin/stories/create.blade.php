...
@section('title', 'Создать рассказ - Obake')
        ...
        <form method="POST" action="{{ route('admin.stories.store') }}">
            ...
            <div class="mb-4">
                ...
                <textarea id="description" name="description" rows="3"
                          class="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:border-purple-500">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
