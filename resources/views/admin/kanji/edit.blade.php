...
@section('title', 'Редактировать кандзи - Obake')

@section('content')
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-8">
        <h1 class="text-4xl font-bold text-purple-400">✏️ Редактировать кандзи</h1>
    </div>
    ...
    <div class="bg-gray-800 rounded-lg shadow-lg p-6">
        <form action="{{ route('admin.kanji.update', $kanji->id) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <input type="text" name="kanji" id="kanji" value="{{ old('kanji', $kanji->kanji) }}" required
            ...
            <div class="mb-6">
                ...
                <input type="text" name="translation_ru" id="translation_ru" value="{{ old('translation_ru', $kanji->translation_ru) }}" required
            ...
            <div class="mb-6">
                ...
                <textarea name="alternative_translations" id="alternative_translations" rows="2"
                       class="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:border-purple-500"
                       placeholder="теперь, в данный момент, ныне (через запятую, точку с запятой или с новой строки)">{{ old('alternative_translations', $kanji->alternative_translations) }}</textarea>
            ...
            <div class="mb-6">
                ...
                <select name="jlpt_level" id="jlpt_level"
                    ...
                    <option value="5" {{ old('jlpt_level', $kanji->jlpt_level) == '5' ? 'selected' : '' }}>N5 (Начальный)</option>
                    <option value="4" {{ old('jlpt_level', $kanji->jlpt_level) == '4' ? 'selected' : '' }}>N4 (Базовый)</option>
                    <option value="3" {{ old('jlpt_level', $kanji->jlpt_level) == '3' ? 'selected' : '' }}>N3 (Средний)</option>
                    <option value="2" {{ old('jlpt_level', $kanji->jlpt_level) == '2' ? 'selected' : '' }}>N2 (Выше среднего)</option>
                    <option value="1" {{ old('jlpt_level', $kanji->jlpt_level) == '1' ? 'selected' : '' }}>N1 (Продвинутый)</option>
                </select>
                @error('jlpt_level')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-6">
                ...
                <textarea name="description" id="description" rows="3"
                          class="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:border-purple-500"
                          placeholder="Дополнительная информация о кандзи (опционально)">{{ old('description', $kanji->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="number" name="stroke_count" id="stroke_count" value="{{ old('stroke_count', $kanji->stroke_count) }}" min="1" max="50"
                       class="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:border-purple-500"
                       placeholder="4">
                @error('stroke_count')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
            ...
            <div class="mb-6">
                <label class="flex items-center">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $kanji->is_active) ? 'checked' : '' }}
