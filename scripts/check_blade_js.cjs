...
for (let i = 0; i < s.length; i++) {
  ...
  if (opening.has(ch)) {
    stack.push({ch, i});
    ops.push({op: 'push', ch, i, depth: stack.length, line: s.slice(0,i).split(/\n/).length});
  } else if (closing.has(ch)) {
    if (stack.length === 0) {
      // unmatched closing
      const before = s.slice(Math.max(0, i-80), i+80);
      const line = s.slice(0, i).split(/\n/).length;
      console.error(`UNMATCHED CLOSING '${ch}' at index ${i} (approx line ${line})\n...${before}...`);
      console.error('Stack (most recent first):', stack.slice(-10).reverse());
      console.error('Recent operations (most recent last):');
      ops.slice(-60).forEach(o => console.error(o));
      process.exit(2);
    }
    const last = stack.pop();
    ops.push({op: 'pop', expect: pairs[last.ch], found: ch, i, depth: stack.length, line: s.slice(0,i).split(/\n/).length});
    if (pairs[last.ch] !== ch) {
      const line = s.slice(0, i).split(/\n/).length;
